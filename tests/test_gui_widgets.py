...
def _click_at(ax, xdata, ydata, button=MouseButton.LEFT):
...
def pytest_approx_pi_over_two() -> float:
    # Defined as a plain float so equality matches the value emitted by atan2.
...
def test_set_val_does_not_canonicalize():
    ...
    np.testing.assert_allclose(received, [3 * math.pi])
    pyplot.close(fig)
